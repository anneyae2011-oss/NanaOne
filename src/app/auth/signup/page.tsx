...
import { Phone, User, ArrowRight, ShieldCheck, ChevronRight } from 'lucide-react';
        ...
        <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <label style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <User size={14} /> Choose a Unique Username
            </label>
            <input 
              className="input-field" 
              placeholder="e.g. nana_pro" 
              value={username}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <label style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Phone size={14} /> Phone Number (Verified via Twilio)
            </label>
            <input 
              className="input-field" 
            ...
            <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <ShieldCheck size={10} /> Real numbers only. VOIP/Virtual numbers will be blocked.
            </p>
          </div>
          ...
          <button className="btn-primary" type="submit" disabled={loading} style={{ marginTop: '12px' }}>
            {loading ? 'Verifying Number...' : 'Sign Up & Enter Dashboard'}
